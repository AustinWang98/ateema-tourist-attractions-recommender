...
def _synthetic_frames_from_events(data: Path, events_path: Path):
    ...
    ev = _normalise_events(pd.read_csv(events_path, low_memory=False))
...
def _load_frames():
    ...
    events_path = Path(os.getenv("EVENTS_PATH", data / "demo_events.csv"))
    ...
    print(f"No private features CSV; building interactions from {events_path}")
    return _synthetic_frames_from_events(data, events_path)
...
def main() -> None:
    ...
    if frames.events is None or frames.events.empty:
        print("No event data found; configure EVENTS_PATH or use the demo fixture.")
        sys.exit(1)
